    ...
    api: function (View) {
        View.prototype.replaceWithSynonyms = function (nounsDict, adjsDict, verbsDict, adverbsDict) {
            ...
            if (verbsDict) {
                ...
                m2.map(v => {
                    console.log(v.text());
                    if (v.match('(@hasHyphen|@hasComma|@hasQuote|@hasPeriod|@hasExclamation|@hasQuestionMark|@hasEllipses|@hasSemicolon|@hasColon|@hasContraction)').found) {
                        return v;
                    }
...
const text = `Now that you understand postpartum hair loss, let's explore prevention and restoration strategies. We'll focus on two key areas: home remedies and beneficial shampoos and supplements.`;
